...
final class LocationManager: NSObject {
    ...
    func calculateDistance(latitude : String?, longitude : String?) -> Double{
        let latitude = Double(latitude ?? "")
        let longitude = Double(longitude ?? "")
        let destLoc   = CLLocation(latitude: latitude ?? 0.0, longitude: longitude ?? 0.0)
        let calcDistance  = self.lastLocation?.distance(from:destLoc ) ?? 0.0
        let km = String.init(format:"%.2f", calcDistance/1000)
